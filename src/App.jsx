...
function cellStyle(metadata, styles) { const style = metadata?.style ? styles?.[metadata.style] : null; const font = style?.font || {}; const fill = style?.fill || {}; const alignment = style?.alignment || {}; return { color: font.color || undefined, backgroundColor: fill.color || undefined, fontFamily: font.name ? `'${font.name}', ui-monospace, monospace` : undefined, fontSize: font.size ? `${font.size}pt` : undefined, fontWeight: font.bold ? 700 : undefined, fontStyle: font.italic ? 'italic' : undefined, textAlign: alignment.horizontal || undefined, verticalAlign: alignment.vertical || undefined, whiteSpace: alignment.wrapText ? 'normal' : 'nowrap', transform: alignment.textRotation && alignment.textRotation !== '0' ? `rotate(${alignment.textRotation}deg)` : undefined, paddingLeft: alignment.indent ? `${Number(alignment.indent) * 0.5}rem` : undefined } }
